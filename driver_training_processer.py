...
def run_outlier_model(collect_train_file_list,obs_stats,all_train_obs,all_test_obs,test_performance):
    
    dead_model_params=dead_model_training(collect_train_file_list,obs_stats,all_train_obs)
    print(f"$$$$ Outlier combined model training $$$$$$")
    dead_model=combine_trained_models(collect_train_file_list, dead_model_params)
    
    train_obs_probs_dead_model=calculate_class_probability(dead_model,collect_train_file_list,obs_stats,all_train_obs)
    ...
    if test_performance==True:
        test_obs_probs_dead_model=calculate_class_probability(dead_model,collect_train_file_list,obs_stats,all_test_obs)
...
def run_bayesian_model(collected_file_lists,obs_stats,all_train_obs,all_test_obs,margin_adjustment,test_performance):
...
def infer_with_trained_bayesian_model(collect_infer_file_lists,infer_obs_stats,all_infer_obs,dead_model,alive_model,bayesian_model_without_threshold):
...
def infer_with_trained_outlier_model(collect_infer_file_lists,infer_obs_stats,all_infer_obs,dead_model,outlier_model_threshold):
        

    infer_obs_probs_dead_model=calculate_class_probability(dead_model,collect_infer_file_lists,infer_obs_stats,all_infer_obs)
    infer_probs_predicted_for_dead_model=outlier_model_threshold.predict_probabilities_dictionary_update(infer_obs_probs_dead_model)
    
    plot_confusion_matrix(infer_probs_predicted_for_dead_model, "Infer","Oranges", "Bayesian")
    all_infer_obs_labeled=get_obs_labeled_by_prediction(all_infer_obs,infer_probs_predicted_for_dead_model)
    return all_infer_obs_labeled
