...
class PreProcessingObservations:
    ...
    def analyze_object(self,o):
        ...
        for i in range(1, len(o)):
            dx.append(o[i][0] - o[i-1][0])
            dy.append(o[i][1] - o[i-1][1])
    ...
    def analyze(self,objects):
        for objectid, tracks in objects.items():
            print()
            print(f'analyzing {objectid} with {len(objects[objectid][TRACKING_DATA])} objects')
            self.analyze_object(objects[objectid][TRACKING_DATA])


    def get_track_length_statistics(self,labeled_observations):
        """
        Compute statistics about track lengths in the labeled observations dictionary.
        
        Parameters:
        - labeled_observations: Dictionary with structure:
            {object_id: {TRACKING_DATA: [(x1,y1,frame1),...,(xn,yn,framen)], 
                         TRUE_LABEL: label}}
        
        Returns:
        - Dictionary with statistics: max, min, mean, median, total_objects
        """
        if not labeled_observations:
            return {
                'max_length': 0,
                'min_length': 0,
                'mean_length': 0,
        ...
        # Extract all track lengths
        track_lengths = [
            len(data[TRACKING_DATA]) 
            for data in labeled_observations.values()
        ]
        ...
        stats = {
            ...
            'mean_length': numpy.mean(track_lengths),
            'median_length': numpy.median(track_lengths),
            'total_objects': len(track_lengths),
            'all_lengths': track_lengths
        }
        
        return stats


    def print_track_statistics(self, labeled_observations, dataset_name):
        """
        Print track length statistics in a formatted way.
        
        Parameters:
        - labeled_observations: Dictionary with labeled observations
        - dataset_name: Name of the dataset for display
        """
        stats = self.get_track_length_statistics(labeled_observations)
        
        print(f"\n{'='*60}")
        print(f"TRACK LENGTH STATISTICS - {dataset_name}")
        print(f"{'='*60}")
        print(f"Total objects:          {stats['total_objects']}")
        print(f"Maximum track length:   {stats['max_length']}")
        print(f"Minimum track length:   {stats['min_length']}")
        print(f"Average track length:   {stats['mean_length']:.2f}")
        print(f"Median track length:    {stats['median_length']:.2f}")
        print(f"{'='*60}\n")
        
        return stats


    def get_track_statistics_by_label(self,labeled_observations):
        """
        Get track length statistics separated by label (motile vs non-motile).
        ...
        for obj_id, data in labeled_observations.items():
            track_length = len(data[TRACKING_DATA])
            
            if data[TRUE_LABEL] == MOTILE:
                motile_lengths.append(track_length)
            else:
                notmotile_lengths.append(track_length)
        
        stats = {
            'motile': {
                'count': len(motile_lengths),
                'max': max(motile_lengths) if motile_lengths else 0,
                'min': min(motile_lengths) if motile_lengths else 0,
                'mean': numpy.mean(motile_lengths) if motile_lengths else 0,
                'median': numpy.median(motile_lengths) if motile_lengths else 0,
            },
            'notmotile': {
                'count': len(notmotile_lengths),
                'max': max(notmotile_lengths) if notmotile_lengths else 0,
                'min': min(notmotile_lengths) if notmotile_lengths else 0,
                'mean': numpy.mean(notmotile_lengths) if notmotile_lengths else 0,
                'median': numpy.median(notmotile_lengths) if notmotile_lengths else 0,
            }
        }
    ...
    def print_track_statistics_by_label(self,labeled_observations, dataset_name):
        """
        Print track length statistics separated by label.
        
        Parameters:
        - labeled_observations: Dictionary with labeled observations
        - dataset_name: Name of the dataset for display
        """
        stats = self.get_track_statistics_by_label(labeled_observations)
        
        print(f"\n{'='*60}")
        print(f"TRACK LENGTH STATISTICS BY LABEL - {dataset_name}")
        print(f"{'='*60}")
        
        print(f"\nMOTILE OBJECTS:")
        print(f"  Count:              {stats['motile']['count']}")
        print(f"  Maximum length:     {stats['motile']['max']}")
        print(f"  Minimum length:     {stats['motile']['min']}")
        print(f"  Average length:     {stats['motile']['mean']:.2f}")
        print(f"  Median length:      {stats['motile']['median']:.2f}")
        
        print(f"\nNON-MOTILE OBJECTS:")
        print(f"  Count:              {stats['notmotile']['count']}")
        print(f"  Maximum length:     {stats['notmotile']['max']}")
        print(f"  Minimum length:     {stats['notmotile']['min']}")
        print(f"  Average length:     {stats['notmotile']['mean']:.2f}")
        print(f"  Median length:      {stats['notmotile']['median']:.2f}")
